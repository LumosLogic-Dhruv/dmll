...
const Navbar = () => {
  ...
  return (
    <>
      {" "}
      <header
        className={`fixed top-0 left-0 right-0 transition-all duration-300 ${isScrolled ? "bg-background/80 backdrop-blur-xl border-b border-border shadow-sm py-3" : "bg-transparent py-5"}`}
        style={{ zIndex: 50 }}
        data-navbar="true"
      >
        {" "}
              ...
              <div key={link.path} className="relative">
                ...
                {link.hasMegaMenu ? (
                  <a
                    ...
                    className={`flex items-center gap-1 px-4 py-2 text-sm font-medium transition-all duration-200 link-underline ${location.pathname.startsWith("/services") || isMegaMenuOpen ? "text-foreground" : "text-muted-foreground hover:text-foreground"}`}
                  >
                    {" "}
                    {link.name}{" "}
                    <ChevronDown
                  ...
                  <Link
                    to={link.path}
                    className={`px-4 py-2 text-sm font-medium transition-all duration-200 link-underline ${location.pathname === link.path ? "text-foreground" : "text-muted-foreground hover:text-foreground"}`}
                  >
                    {" "}
                    {link.name}{" "}
                  </Link>
                ...
                <div className="grid grid-cols-12 gap-8">
                  ...
                  <div className="col-span-2 border-r border-border pr-6">
                    ...
                    <p className="text-sm text-muted-foreground mb-6">
                      {" "}
                      Campaign management, SEO, and CRO that improve your
                      marketing KPIs.{" "}
                    </p>{" "}
                    ...
                    {link.hasMegaMenu ? (
                      <div className="border-b border-border">
                        ...
                        >
                          ...
                          {link.name}{" "}
                          <ChevronDown
                            className={`w-5 h-5 transition-transform ${activeAccordion === link.name ? "rotate-180" : ""}`}
                          />{" "}
                        </button>{" "}
                        <AnimatePresence>
                          {" "}
                      ...
                      <Link
                        to={link.path}
                        className={`block py-4 text-lg font-medium border-b border-border ${location.pathname === link.path ? "text-foreground" : "text-muted-foreground"}`}
                      >
                        {" "}
                        {link.name}{" "}
                      </Link>
